# This Makefile is for the API::Kaltura extension to perl.
#
...
#     ABSTRACT => q[Kaltura API implementation.]
#     AUTHOR => [q[J. Eric Ellis (jellisii@gmail.com)]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     NAME => q[API::Kaltura]
#     PREREQ_PM => { Carp=>q[0], Test::Simple=>q[0.44], XML::Twig=>q[0], LWP=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/API/Kaltura.pm]
...
DFSEP = $(DIRFILESEP)
NAME = API::Kaltura
NAME_SYM = API_Kaltura
VERSION = 0.02EXPERIMENTAL
VERSION_MACRO = VERSION
VERSION_SYM = 0_02EXPERIMENTAL
DEFINE_VERSION = -D$(VERSION_MACRO)=\"$(VERSION)\"
XS_VERSION = 0.02EXPERIMENTAL
...
MAN3EXT = 3pm
INSTALLDIRS = site
INSTALL_BASE = /home/eellis/perl5
DESTDIR = 
PREFIX = $(INSTALL_BASE)
...
PARENT_NAME = API
DLBASE = $(BASEEXT)
VERSION_FROM = lib/API/Kaltura.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/API/Kaltura.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/API
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/API
...
TO_INST_PM = lib/API/Kaltura.pm \
	lib/API/Kaltura/CuePoint.pm

PM_TO_BLIB = lib/API/Kaltura/CuePoint.pm \
	blib/lib/API/Kaltura/CuePoint.pm \
	lib/API/Kaltura.pm \
	blib/lib/API/Kaltura.pm
...
DISTNAME = API-Kaltura
DISTVNAME = API-Kaltura-0.02EXPERIMENTAL
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  lib/API/Kaltura.pm $(INST_MAN3DIR)/API::Kaltura.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''Kaltura API implementation.'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''J. Eric Ellis (jellisii@gmail.com)'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: API-Kaltura' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'version: 0.02EXPERIMENTAL' >> META_new.yml
	$(NOECHO) $(ECHO) 'x_serialization_backend: '\''CPAN::Meta::YAML version 0.016'\''' >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) '{' > META_new.json
	$(NOECHO) $(ECHO) '   "abstract" : "Kaltura API implementation.",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "J. Eric Ellis (jellisii@gmail.com)"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "API-Kaltura",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   "version" : "0.02EXPERIMENTAL",' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Kaltura API implementation.</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>J. Eric Ellis (jellisii@gmail.com)</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/API/Kaltura/CuePoint.pm blib/lib/API/Kaltura/CuePoint.pm \
	  lib/API/Kaltura.pm blib/lib/API/Kaltura.pm 
